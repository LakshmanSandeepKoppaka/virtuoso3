	...
	<body>
		...
		<main>
			<div class="container" style="padding: 30px;">
				<h4>UPDATE ALBUM DETAILS</h4>
				<%@include file="dbconnection.jsp"%>
				<%@page import="java.sql.PreparedStatement"%>
				<%@page import="java.sql.ResultSet"%>
				<%
				String aid = request.getParameter("aid");
				PreparedStatement ps = con.prepareStatement("SELECT * FROM album WHERE id = '" + aid + "'");
				ResultSet rs = ps.executeQuery();
				if(rs.next()) { %>
				<form action="crudoperations.jsp" method="post">
					<input type="hidden" name="role" value="album_detail_edit">
					<input type="hidden" name="aid" value="<%=aid%>">
					<div class="form-row">
						<div class="col-md-6 mb-3">
							<label for="al_name">Album Name</label>
							<input type="text" class="form-control" id="al_name" name="al_name" value="<%=rs.getString("al_name")%>" placeholder="Enter Album Name" required>
						</div>
					</div>
					<div class="form-row">
						<div class="col-md-6 mb-3">
							<label for="al_artist_name">Artist Name</label>
							<input type="text" class="form-control" id="al_artist_name" name="al_artist_name" value="<%=rs.getString("al_artist_name")%>" placeholder="Enter Artist Name" required>
						</div>
					</div>
					<div class="form-row">
						<div class="col-md-6 mb-3">
							<label for="al_price">Price</label>
							<input type="number" class="form-control" id="al_price" name="al_price" value="<%=rs.getString("al_price")%>" placeholder="Enter Price" required>
						</div>
					</div>
					<div class="form-row">
						<div class="col-md-6 mb-3">
							<label for="al_desc">Description</label>
							<textarea class="form-control" name="al_desc" id="al_desc" placeholder="Enter Description" rows="6" required><%=rs.getString("al_desc")%></textarea>
						</div>
					</div>
