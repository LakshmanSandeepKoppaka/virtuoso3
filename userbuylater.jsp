	...
	<body>
		...
		<main>
			<div class="container">
				<div class="card mb-3">
					<div class="row no-gutters">
						<div class="col-md-12">
							<div class="card-body">
								<h2 class="card-title text-center">SAVED PRODUCTS</h2>
							</div>
						</div>
					</div>
				</div>
			...
			<div class="container" style="margin-top: 50px; margin-bottom: 60px;">
				...
				PreparedStatement ps = con.prepareStatement("SELECT * FROM user_product u INNER JOIN product p on u.product_id = p.id WHERE u.pr_status = 'Save For Later' AND u.user_id = '" + u_id + "'");
						...
						<div class="col-md-8" style="background-color: mintcream;">
							<div class="card-body">
								...
								if(rs.getString("p.pr_stock").equals("In Stock")) { %>
									<a href="crudoperations.jsp?pr_id=<%=rs.getString(1)%>&role=later_product_add_cart" style="text-decoration: none;">
										<p>Add To Cart</p>
									</a>
								<% } else { %>
									<h4 class="text-center" style="color: orangered;">Out of Stock</h4>
								<% } %>
								<p class="card-text"><b>Name: </b> <%=rs.getString("pr_name")%></p>
								<p class="card-text"><b>Quantity Purchased: </b> <%=rs.getString("user_qnty")%></p>
								<p class="card-text"><b>Total Cost: </b> <%=rs.getString("total_price")%></p>
							</div>
						</div>
					</div>
				</div>
				<% } 
				if(count == 0) { %>
					<h4 class="text-center" style="color: orangered;">No products saved</h4>
				<% } %>
			</div>
