	...
	<body>
		...
		<main>
			<div class="container">
				<div class="card mb-3">
					<div class="row no-gutters">
						<div class="col-md-12">
							<div class="card-body">
								<h2 class="card-title text-center">PURCHASED PRODUCTS</h2>
							</div>
						</div>
					</div>
				</div>
			...
			<div class="container" style="margin-top: 50px; margin-bottom: 60px;">
				...
				PreparedStatement ps = con.prepareStatement("SELECT * FROM user_product u INNER JOIN product p on u.product_id = p.id WHERE u.pr_status = 'Purchased' AND u.user_id = '" + u_id + "' ORDER BY u.id DESC");
					...
					<div class="row no-gutters">
						...
						<div class="col-md-8" style="background-color: mintcream;">
							<div class="card-body">
								...
								<p class="card-text"><b>Shipping Address: </b> <%=rs.getString("shipping_address")%></p>
							</div>
						</div>
					</div>
				</div>
				<% } 
				if(count == 0) { %>
					<h4 class="text-center" style="color: orangered;">No purchase history</h4>
				<% } %>
			</div>
